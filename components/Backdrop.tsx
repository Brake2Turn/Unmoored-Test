 ...
 *
 * These are fixed rather than rolled. The title screen is the first thing seen
 * every launch, and a planet that reshuffled its continents each time would
 * read as a glitch rather than as a world.
 */
const SURFACE: {
  x: number;
  y: number;
 ...
 * rim pushed toward the light, then the floor laid over all but its crescent,
 * which is what reads as a dished hollow rather than a flat spot.
 */
 ...
 * round, which reads as a bubble; this is far enough that the floor covers
 * everything but a crescent on the lit side.
 */
 ...
 * surface is built the same way, out of flat shapes at low opacity: React
 * Native SVG has no dependable blur or noise filter, so texture has to be
